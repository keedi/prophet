...
=method changesets_for_record { uuid => $uuid, type => $type, limit => $int }
...
sub create_record {
    ...
    $change->add_prop_change(
        name => $_,
        old  => undef,
        new  => $args{props}->{$_}) for (keys %{$args{props}});
