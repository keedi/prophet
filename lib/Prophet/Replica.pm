...
Determines what replica class to use and instantiates it. Returns the new
replica object.
...
sub _url_to_replica_class {
    ...
    for my $class ( ref( $args{app_handle} ) . "::Replica::" . $scheme,
...
=method import_changesets { from => L<Prophet::Replica> ... }

Given a L<Prophet::Replica> to import changes from, traverse all the changesets
we haven't seen before and integrate them into this replica.

This routine calls L<traverse_changesets> on the 'from' replica, passing in the
most recent changeset the current replica has seen and a callback routine which
calls L<integrate_changeset> on the local replica.

That callback itself takes a callback, L<after_integrate_changeset> , which a
replica implementation can use to perform some action after a changeset is
integrated into a peer.  L<after_integrate_changeset> takes a paramhash,
currently with only a single key, 'changeset'.
...
sub import_changesets {
    ...
    $self->log_debug( "Integrating changesets from "
          . $source->uuid
          . " after "
          . $self->last_changeset_from_source( $self->uuid ) );
...
=method import_resolutions_from_remote_replica { from => L<Prophet::Replica> ... }

Takes a L<Prophet::Replica> object (and possibly some optional arguments) and
imports its resolution changesets into this replica's resolution database.

Returns immediately if either the source replica or the target replica lack a
resolution database.
...
=method integrate_changeset L<Prophet::ChangeSet>
...
before it) from both the peer who sent it to us AND the replica which
...
sub integrate_changeset {
    ...
    $self->log_debug( "Considering changeset "
    ...
    if ( !$self->should_accept_changeset($changeset) ) {
        ...
        $self->log_debug(
                "Integrating conflicting changeset "
              . $changeset->original_sequence_no
              . " from "
              . $self->display_name_for_replica(
                $changeset->original_source_uuid
              )
        );
        $args{conflict_callback}->($conflict) if $args{'conflict_callback'};
        $conflict->resolvers( [ sub { $args{resolver}->(@_) } ] )
          if $args{resolver};
...
=method record_changeset_and_integration L<Prophet::ChangeSet>
...
=method last_changeset_from_source $SOURCE_UUID
...
sub last_changeset_from_source {
    ...
    my $changeset_num =
      $self->fetch_local_metadata( 'last-changeset-from-' . $source );

    # 0 is a valid changeset #
    return defined $changeset_num ? $changeset_num : -1;
}

=method has_seen_changeset { source_uuid => <uuid>, sequence_no => <int> }
...
sub has_seen_changeset {
    ...
    elsif ( $self->last_changeset_from_source( $args{source_uuid} ) >=
        $args{sequence_no} )
    {
        $self->log_debug(
            "\t  - We have seen this or a more recent changeset from remote.");
        return 1;
    } else {
        return;
    }
...
=method changeset_will_conflict L<Prophet::ChangeSet>
...
=method conflicts_from_changeset L<Prophet::ChangeSet>
...
sub conflicts_from_changeset {
    ...
    return unless $conflict->has_conflicting_changes;
...
=method should_accept_changeset { from => L<Prophet::Replica>, changeset => L<Prophet::ChangeSet> }
...
sub should_accept_changeset {
    my $self = shift;
    my ($changeset) =
      validate_pos( @_, { changeset => { isa => 'Prophet::ChangeSet' } } );

    $self->log_debug( "Should I accept "
          . $changeset->original_sequence_no
          . " from "
          . $self->display_name_for_replica( $changeset->original_source_uuid )
    );
...
=method fetch_changesets { after => SEQUENCE_NO }
...
See also L<traverse_changesets> for replica implementations to provide streamly
interface.
...
Given a UUID => LUID hash mapping, return a new unused LUID (one higher than
the mapping's current highest luid).
...
Returns a reference to the parsed JSON contents of the file given by C<$PATH>
in the replica's userdata directory.
...
serializes C<$VALUE> to JSON and writes it to the file given by C<$PATH> in the
replica's userdata directory, creating parent directories as necessary.
...
Returns a list of cached upstream replica URLs, or an empty list if there are
no cached URLs.
...
sub _read_cached_upstream_replicas {
    my $self = shift;
    return
      @{ $self->_do_userdata_read( $self->_upstream_replica_cache_file, '[]' )
          || [] };
}

=head3 _write_cached_upstream_replicas @REPLICAS

writes the replica URLs given by C<@REPLICAS> to the upstream replica cache
file.
...
sub _write_cached_upstream_replicas {
    my $self     = shift;
    my @replicas = @_;
    return $self->_do_userdata_write( $self->_upstream_replica_cache_file,
        [@replicas] );
}

=head3 _guid2luid_file

The file in the replica's userdata directory which contains a serialized JSON
UUID => LUID hash mapping.
...
sub traverse_changesets {
    ...
    Carp::confess
      "$class has failed to implement a 'traverse_changesets' method for their replica type.";
}
...
sub record_resolutions {
    ...
    $self->_unimplemented(
        "record_resolutions (since there is no writable handle)")
...
Inside an edit (transaction), integrate all changes in this changeset and then
call the _after_record_changes() hook.
...
Changes can have the following types: - add_file - add_dir - update_file -
delete
...
sub integrate_change {
    ...
    if ( $change->change_type eq 'add_file' ) {
        $self->log_debug(
            "add_file: " . $change->record_type . " " . $change->record_uuid );
        $self->create_record(
...
This routine records the immediately upstream and original source uuid and
sequence numbers for this changeset. Prophet uses this data to make sane
choices about later replay and merge operations
...
sub record_integration_of_changeset {
    ...
    if (
        $changeset->original_source_uuid ne $self->uuid
        && (
    ...
    if ( $changeset->source_uuid ) {
        if ( $self->last_changeset_from_source( $changeset->source_uuid ) <
...
sub record_last_changeset_from_replica {
    ...
    return $self->store_local_metadata( 'last-changeset-from-' . $uuid,
        $sequence );
}
...
sub log_debug {
    ...
    $self->app_handle->log_debug(
        $self->display_name_for_replica . ": " . $msg );
}
...
=method store_local_metadata KEY => VALUE

Takes a key and a value.

Store some bit of metadata in a durable local datastore.  Metadata isn't
propagated when replicas are synced.

Returns true or false.

=method fetch_local_metadata KEY
