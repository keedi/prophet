...
=method register_reference $class, $accessor, $foreign_class, @args
...
=method register_collection_reference $accessor, $collection_class, by => $key_in_model
...
=method register_record_reference $accessor, $record_class, by => $key_in_model
...
=method create { props => { %hash_of_kv_pairs } }
...
=method load { uuid => $UUID } or { luid => $UUID }
...
=method set_prop { name => $name, value => $value }
...
=method validate_props $propsref
...
=method canonicalize_props $propsref
...
=method default_props $props_ref
...
=method default_prop_original_replica
...
=method validate_prop_from_recommended_values 'prop', $argsref
...
=method recommended_values_for_prop 'prop'
...
=method _atomize_summary_format [$format]
...
=method format_atom $string => $value
...
=method record_reference_methods
...
=method collection_reference_methods
...
=method new  { handle => Prophet::Replica, type => $type }
